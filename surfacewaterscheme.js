...
// Load CSV data
Papa.parse('downloads/combined_surfaceWaterScheme.csv', {
    download: true,
    header: true,
    complete: function(results) {
        allData = results.data.filter(row => row['serial_number']); // Filter out empty rows
        populateTalukas();
        applyFilters();
    },
    error: function(err) {
        console.error("Error loading CSV:", err);
        scheduleContainer.innerHTML = '<div style="color: red;">Error loading data. Please ensure the file exists at downloads/combined_surfaceWaterScheme.csv and you are running this via a local server.</div>';
    }
});

function populateTalukas() {
    const talukas = [...new Set(allData.map(row => row['block_tehsil']))].filter(Boolean).sort();
...
function populateVillages(selectedTaluka) {
    ...
    if (selectedTaluka !== 'all') {
        filteredForVillages = allData.filter(row => row['block_tehsil'] === selectedTaluka);
    }
...
function applyFilters() {
    ...
    filteredData = allData.filter(row => {
        const matchesTaluka = selectedTaluka === 'all' || row['block_tehsil'] === selectedTaluka;
...
function displayCurrentItem() {
    ...
    // Determine scheme type detail
    const schemeTypeDetail = item['scheme_type_surface_flow']
        ? `Flow - ${item['scheme_type_surface_flow']}`
        : item['scheme_type_surface_lift']
  ...
  scheduleContainer.innerHTML = `
        <div class="schedule-header">
            ...
            <h2>MI CENSUS - SURFACE WATER SCHEME</h2>
        ...
        <div class="identification-box">
            ...
            <div class="grid-row">
                <div class="grid-item"><span class="label">(a) State</span><span class="value">${item.state || 'GOA'}</span></div>
                <div class="grid-item"><span class="label">(b) District</span><span class="value">${item.district || ''}</span></div>
                <div class="grid-item"><span class="label">(c) Block/Sub-District</span><span class="value">${item.block_tehsil || ''}</span></div>
            </div>
            <div class="grid-row">
                <div class="grid-item"><span class="label">(d) Village</span><span class="value">${item.village || ''}</span></div>
        ...
        <div class="two-col">
            <div>
                ...
                <div class="info-item">
                    ...
                    <span class="value">${schemeTypeDetail}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="value">${item.scheme_owner_social_status || 'N/A'} / ${item.gender_name || 'N/A'}</span>
                </div>
                <div class="info-item">
            ...
            <div>
                ...
                <div class="info-item">
                    ...
                    <span class="value">${item.maintainence_cost || '0'}</span>
                </div>
                <div class="info-item">
        ...
        <div class="two-col">
            <div>
                ...
                <div class="info-item">
                    <span class="label">Scheme Recharge of Ground Water</span>
                    <span class="value">${item.scheme_recharge_of_ground_water || 'N/A'}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    <span class="label">Villages Covered</span>
                    <span class="value">${item.villages_covered || 'N/A'}</span>
                </div>
            </div>
            <div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="value">${item.scheme_under_utilisation || 'N/A'}</span>
                </div>
                <div class="info-item">
                    <span class="label">Reason for Under-utilisation</span>
                    <span class="value">${item.reason_for_under_utilisation || 'N/A'}</span>
                </div>
            </div>
        </div>

        <div class="section-title">V. Storage &amp; Other</div>
        <div class="two-col">
            <div>
                <div class="info-item">
                    <span class="label">Scheme Designed Storage</span>
                    <span class="value">${item.scheme_designed_storage || 'N/A'}</span>
                </div>
                <div class="info-item">
                    <span class="label">Filled Up Storage</span>
                    <span class="value">${item.filled_up_storage || 'N/A'}</span>
                </div>
                <div class="info-item">
                    <span class="label">Status of Filled Up Storage Space</span>
                    <span class="value">${item.filled_up_storage_space || 'N/A'}</span>
                </div>
                <div class="info-item">
                    <span class="label">Associated Water Body Unique ID</span>
                    <span class="value">${item.wbs_unique_id || 'N/A'}</span>
                </div>
            </div>
            <div>
                <div class="info-item">
...
function performSearch() {
    ...
    const index = allData.findIndex(row => row['serial_number'] === searchTerm);
    if (index !== -1) {
